...
/**
* A sequence of PayloadA instances.
*/
public final class PayloadASeq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public PayloadASeq() {
        super(PayloadA.class);
    }

    public PayloadASeq (int initialMaximum) {
        super(PayloadA.class, initialMaximum);
    }

    public PayloadASeq (Collection elements) {
        super(PayloadA.class, elements);
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, PayloadA.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' PayloadA objects (beyond the original size of this object)
