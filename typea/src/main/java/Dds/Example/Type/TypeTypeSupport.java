...
public class TypeTypeSupport extends TypeSupportImpl {
    // -----------------------------------------------------------------------
    // Private Fields
    // -----------------------------------------------------------------------

    private static final String TYPE_NAME = "Dds::Example::Type::Type";

    private static final char[] PLUGIN_VERSION = {2, 0, 0, 0};     
    private static final TypeTypeSupport _singleton
    = new TypeTypeSupport();
    ...
    */
    public static TypeTypeSupport get_instance() {
        return _singleton;
    }

    public static TypeTypeSupport getInstance() {
        return get_instance();
    }

    public static TypeCode getTypeCode(){
        return TypeTypeCode.VALUE;
    }
    ...
    */
    public Object copy_data(Object destination, Object source) {
        ...
        Type typedDst = (Type) destination;
        Type typedSrc = (Type) source;
    ...
    public long get_serialized_sample_max_size(Object endpoint_data,boolean include_encapsulation,short encapsulation_id,long currentAlignment) {
        ...
        currentAlignment += Dds.Example.Type.PayloadATypeSupport.get_instance().get_serialized_sample_max_size(endpoint_data,false, encapsulation_id,0);
        if (include_encapsulation) {
        ...
        Object sample) 
    {
        ...
        if (typedSrc.payloadA != null) {
            currentAlignment += Dds.Example.Type.PayloadATypeSupport.get_instance().get_serialized_sample_size(
                endpoint_data,false,encapsulation_id,0,typedSrc.payloadA);
        }
    ...
    short encapsulation_id, boolean serialize_sample, Object endpoint_plugin_qos) {
        ...
        if(serialize_sample) {
            ...
            if (typedSrc.payloadA != null) {
                Dds.Example.Type.PayloadATypeSupport.get_instance().serialize(endpoint_data, typedSrc.payloadA, dst, false, encapsulation_id,true,endpoint_plugin_qos);
            }
            if (dst.useExtendedMemberId == true) {
        ...
        Object endpoint_plugin_qos) 
    {
        ...
        if(deserialize_sample) {
            ...
            try{
                ...
                if (tmpLength > 0) {
                    if (typedDst.payloadA == null) {
                        typedDst.payloadA = 
                        (Dds.Example.Type.PayloadA) Dds.Example.Type.PayloadA.create();
                    }

                    typedDst.payloadA = (Dds.Example.Type.PayloadA)Dds.Example.Type.PayloadATypeSupport.get_instance().deserialize_sample(endpoint_data, typedDst.payloadA, src, false, true, endpoint_plugin_qos);     
                }
                else {
    ...
    Object endpoint_plugin_qos)
    {
        ...
        if (skip_sample) {
            ...
            src.getBuffer().setDesBufferSize((int)(tmpPosition + memberInfo.length));
            if (tmpLength > 0) {
                Dds.Example.Type.PayloadATypeSupport.get_instance().skip(endpoint_data, src, false, true, endpoint_plugin_qos);
            }
            src.getBuffer().setDesBufferSize(tmpSize);
            src.getBuffer().setCurrentPosition((int)(tmpPosition + tmpLength));
    ...
    */
    ...
    int mask) {
        return new TypeDataWriter (native_writer, listener, mask, this);            
    }
    ...
    int mask) {
        ...
        return new TypeDataReader(native_reader, listener, mask, this);   
    ...
    protected TypeTypeSupport() {

        /* If the user data type supports keys, then the second argument
        to the constructor below should be true.  Otherwise it should
        be false. */        

        super(TYPE_NAME,true,TypeTypeCode.VALUE,Type.class,TypeSupportType.TST_STRUCT, PLUGIN_VERSION);

    }

    protected TypeTypeSupport (boolean enableKeySupport) {

        super(TYPE_NAME, enableKeySupport,TypeTypeCode.VALUE,Type.class,TypeSupportType.TST_STRUCT, PLUGIN_VERSION);
    }
}
